...
def flatten(l): # 转换l为字母表
    return list(itertools.chain.from_iterable(l))

seqs = ['ghatmasala', 'nicela', 'chutpakodas']

# make <pad> idx 0 转换sort字母set
...
#<class 'list'>: [[5, 6, 1, 15, 10, 1, 14, 1, 9, 1], [11, 7, 2, 4, 9, 1], [2, 6, 16, 15, 13, 1, 8, 12, 3, 1, 14]]
vectorized_seqs = [[vocab.index(tok) for tok in seq]for seq in seqs]
print("vectorized_seqs", vectorized_seqs)

print([x for x in map(len, vectorized_seqs)])
# get the length of each seq in your batch  # 得到每个序列的长度，建立一个tensor
seq_lengths = torch.LongTensor([x for x in map(len, vectorized_seqs)])

# dump padding everywhere, and place seqs on the left.
# NOTE: you only need a tensor as big as your longest sequence
seq_tensor = Variable(torch.zeros(
    (len(vectorized_seqs), seq_lengths.max()))).long()  # size 是 3 * 11，11是这三个序列的最大长度。
for idx, (seq, seqlen) in enumerate(zip(vectorized_seqs, seq_lengths)): # 把index过后的序列，赋值给seq_tensor，一共三个
...
seq_tensor = seq_tensor[perm_idx] # 按照长度对seq_tensor进行从大到小排序
